...
WindVSODmd <- function(wod) {
  # wod <- final
  twsx11 <- wod[wod$Date >= "2011-01-01" & wod$Date < "2015-01-01" ,]
  twsxH11 <- aggregate(cbind(TotWindMWh, Ontario.Demand, Exports) ~ ym, data = twsx11, sum)
  ...
  plot(twsH11$ym, twsH11$Ontario.Demand, col="blue", axes=T, main="Energy Consumption",xlab="Year+Month",ylab="","s")
